...
export const RecentViewedProvider = ({ children }) => {
    ...
    const updateRecentViewed = (newData) => {
        setRecentViewed((prevRecentViewed) => {
            ...
            const index = prevRecentViewed.findIndex((item) => item._id === newData._id);
            ...
            LocalStorageManager.saveUserData(user._id, KEYS.KEY_RECENT_VIEWED, newRecentViewed);
    ...
    useEffect(() => {
        if (user?.id) {
            const savedData = LocalStorageManager.getUserData(user._id, KEYS.KEY_RECENT_VIEWED);
